...
def normal_probability_plot(sample, fit_color='0.8', **options):
    ...
    plot.plot(*fit, color=fit_color, label='model')

    xs, ys = normal_probability(sample)
    plot.plot(xs, ys, **options)
...
def _cov(xs, ys, meanx=None, meany=None):
    """Computes Cov(X, Y).
...
def _corr(xs, ys):
    ...
    corr = _cov(xs, ys, meanx, meany) / math.sqrt(varx * vary)
...
def least_squares(xs, ys):
    ...
    slope = _cov(xs, ys, meanx, meany) / varx
...
class HypothesisTest(object):
    ...
    def plot_cdf(self, label=None):
        ...
        def vert_line(x):
            ...
            plot.plot([x, x], [0, 1], color='0.8')

        vert_line(self.actual)
        plot.cdf(self.test_cdf, label=label)
